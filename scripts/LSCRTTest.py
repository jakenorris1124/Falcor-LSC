...
def render_graph_PathTracer():
    ...
    g.create_pass('SplitScreenPass', 'SplitScreenPass', {'splitLocation': 0.0, 'showTextLabels': False, 'leftLabel': 'Left side', 'rightLabel': 'Right side'})
    g.add_edge('VBufferRT.vbuffer', 'PathTracerLSC.vbuffer')
    g.add_edge('VBufferRT.mvec', 'PathTracerLSC.mvec')
    g.add_edge('VBufferRT.viewW', 'PathTracerLSC.viewW')
    g.add_edge('SplitScreenPass.output', 'ToneMapper.src')
    g.add_edge('PathTracerLSC.direct', 'SplitScreenPass.leftInput')
    g.add_edge('PathTracerLSC.indirect', 'SplitScreenPass.rightInput')
